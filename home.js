window.onload=function(){
    $(document).ready(function(){
var text_content =
[
                {
                    temp:"48°F - 7mph - 78°",
                    place:"Lake Tahoe, California",
                    subject:"Homewood Mountain & Lake Club",
                    location:"39.08629948516978° N / -120.1605391°W" 
                },
                {
                    temp:"56°F- 6mph-150°",
                    place:"Silicon Valley, California",
                    subject:"Corde Valle",
                    location:"37.067811° N / -121.632827°W" 
                },
                {
                    temp:"75°F-23mph-114°",
                    place:"La Quinta, California",
                    subject:"Madison",
                    location:"33.660962° N / -116.244456°W" 
                },
                {
                    temp:"75°F-23mph-114°",
                    place:"La Quinta, California",
                    subject:"Hideaway",
                    location:"33.667059° N / -116.260879°W" 
                },
                {
                    temp:"71°F-18mph-340°",
                    place:"Las Vegas, Nevada",
                    subject:"The Summit",
                    location:"36.10804° N / -115.349337°W" 
                },
                {
                    temp:"74°F- 1 mph -107°",
                    place:"Scottsdale, Arizona",
                    subject:"Estancia",
                    location:"33.738105° N / -111.8665240°W" 
                },
                {
                    temp:"72°F -1 mph -57°",
                    place:"Scottsdale, Arizona",
                    subject:"Mirabel",
                    location:"33.823760° N / -111.851592°W" 
                },
                {
                    temp:"75 °F- 23 mph -114°",
                    place:"La Quinta, California",
                    subject:"Hideaway",
                    location:"33.667059° N / -116.260879°W" 
                },
                {
                    temp:"72 °F- 23 mph- 114°",
                    place:"Scottsdale, Arizona",
                    subject:"Mirabel",
                    location:"33.823760° N / -111.851592°W" 
                },
                {
                    temp:" 74°F-  2 mph- 207°",
                    place:"Los Cabos, Mexico",
                    subject:"El Dorado",
                    location:"22.995771° N / -109.746423°W" 
                },
                {
                    temp:"74 °F- 2 mph- 29°",
                    place:"Cabo San Lucas, Mexico",
                    subject:"Chileno Bay",
                    location:"22.953650° N / -109.816218°W" 
                },{
                    temp:"56 °F- 11 mph- 310°",
                    place:"British Columbia, Canada",
                    subject:" James Island",
                    location:"48.602830922° N / -123.342998628°W" 
                },{
                    temp:"51 °F- 6 mph- 170°",
                    place:"Coeur d'Alene, Idaho",
                    subject:"Gozzer Ranch",
                    location:"47.617997° N / -116.766169°W" 
                },{
                    temp:"44 °F- 33 mph- 280°",
                    place:"Whitefish, Montana",
                    subject:"Iron Horse",
                    location:"48.449344° N / -114.347434°W" 
                },{
                    temp:"35 °F- 2 mph- 69°",
                    place:"Big Sky, Montana",
                    subject:"Yellowstone Club",
                    location:"45.245981° N / -111.374457°W" 
                },{
                    temp:"73 °F- 14 mph- 80°",
                    place:"Westlake, Texas",
                    subject:"Vaquero",
                    location:"32.965190° N / -97.194715°W" 
                },
                {
                    temp:"79 °F- 5 mph- 180°",
                    place:"Austin, Texas",
                    subject:"Driftwood",
                    location:"30.139339° N / -98.022342°W" 
                },
                {
                    temp:"71 °F- 3 mph- 345°",
                    place:"Guanacaste, Costa Rica",
                    subject:"Zapotal Golf & Beach Club",
                    location:"9.998076° N / -85.305086°W" 
                },
                {
                    temp:"63 °F- 2 mph- 261°",
                    place:"Nashville, Tennessee",
                    subject:"Troubadour",
                    location:"35.828904° N / -86.679158°W" 
                },
                {
                    temp:"54 °F- 4 mph -332°",
                    place:"Cashiers, North Carolina",
                    subject:"Mountaintop",
                    location:"35.106704° N / -83.097106°W" 
                },
                {
                    temp:"82 °F-  4mph- 190°",
                    place:"Naples, Florida",
                    subject:"Naples Beach Club",
                    location:"26.137667°N/-81.807056°W" 
                },
                {
                    temp:"81 °F - 14mph- 175°",
                    place:"Great Guana Cay, Bahamas",
                    subject:"Baker's Bay",
                    location:"26.686966° N / -77.146015°W" 
                },
                {
                    temp:"76 °F- 2 mph- 170°",
                    place:"Rio San Juan, Dominican",
                    subject:"Republic Playa Grande",
                    location:"19.3830° N/-70.034601° W" 
                },
                {
                    temp:"81 °F - 20 mph- 87°",
                    place:"Barbuda, West Indies",
                    subject:"Barbuda Ocean Club",
                    location:"17.549884° N/-61.765587° W" 
                },
                {
                    temp:"54 °F- 0 mph- 346°",
                    place:"Amenia, New York",
                    subject:"Silo Ridge",
                    location:"41.834259° N/-73.571259° W" 
                },
                {
                    temp:"60 °F - 0 mph- 0°",
                    place:"Westhampton Beach, New York",
                    subject:"Dune Deck",
                    location:"40.8093799° N / -72.6458277°W" 
                },
                {
                    temp:"60 °F-  5mph -291°",
                    place:"The Hamptons, New York",
                    subject:"The Hills Golf Club",
                    location:"40.84778° N / -72.589811°W" 
                },
                {
                    temp:"73 °F - 7mph- 347°",
                    place:"Comporta, Portugal",
                    subject:"CostaTerra",
                    location:"38.198750° N / -8.764298°W" 
                },
                {
                    temp:"67 °F - 3 mph -136°",
                    place:"Perthshire, Scotland",
                    subject:"Taymouth Castle",
                    location:"56.595999 ° N / -3.979470°W" 
                },
                {
                    temp:"48 °F- 7 mph- 78°",
                    place:"Dubai South, United Arab Emirates",
                    subject:"Discovery Dunes",
                    location:"39.08629948516978° N / -120.1605391°W" 
                },
                {
                    temp:"71 °F- 12 mph- 86°",
                    place:"Kaua'i, Hawaii",
                    subject:"North Shore Preserve",
                    location:"22.193420° N / -159.361620°W" 
                },
                {
                    temp:"75 °F - 5 mph- 80°",
                    place:"Kohala, Hawaii",
                    subject:"Kūki'o",
                    location:"19.817312° N / -155.998547°W" 
                },
                {
                    temp:"73 °F -6 mph- 79°",
                    place:"Maui, Hawaii",
                    subject:"Makena",
                    location:"20.690308° N / -156.440582°W" 
                },
                {
                    temp:"75 °F- 4 mph- 180°",
                    place:"Hobe Sound, Florida",
                    subject:"Atlantic Fields",
                    location:"27.045829992748335° N / -80.2137008693191°W" 
                }
                       
]
$(".temp1").html(text_content[0].temp)
$(".place1").html(text_content[0].place)
$(".subject1").html(text_content[0].subject)
$(".location1").html(text_content[0].location)
var counter=0;

$(".left-button").click (function(){
    if(counter>0){
        counter =counter-1;    
        $(".temp1").html(text_content[counter].temp)
        $(".place1").html(text_content[counter].place)
        $(".subject1").html(text_content[counter].subject)
        $(".location1").html(text_content[counter].location)
    }
})
$(".right-button").click (function(){
        if(counter<text_content.length-1){
            counter=counter+1;
            $(".temp1").html(text_content[counter].temp)
            $(".place1").html(text_content[counter].place)
            $(".subject1").html(text_content[counter].subject)
            $(".location1").html(text_content[counter].location)
        }
})
 } )}
   